        ...
        try {
	        ...
	        if (notification) {
		        ...
		        try {
			        ...
			        tmgr.create("ÐŸÐ¾Ð·Ð²Ð¾Ð½Ð¸Ñ‚Ðµ ÐºÐ»Ð¸ÐµÐ½Ñ‚Ñƒ, Ð¿Ð¾Ð´Ñ‚Ð²ÐµÑ€Ð´Ð¸Ñ‚Ðµ Ð·Ð°Ð¿Ð¸ÑÑŒ", 
			        						null, 
			        						pos, 
			        						dt, 
			        						dt, 
		        ...
		        try{
		        	String p = "Ð½Ðµ ÑƒÐºÐ°Ð·Ð°Ð½Ð°";
		        	String s = "Ð½Ðµ ÑƒÐºÐ°Ð·Ð°Ð½";
		        	...
		        	for (String em : emails.split(",")) {
			        	BBRMailer.send(em, 
			        			"Barbiny: Íîâàÿ çàïèñü â " + visit.getPos().getTitle(), 
			        			"Âðåìÿ: " + df.format(visit.getTimeScheduled()) + "\n" +
			        			"Èìÿ: " + visit.getUserName() + "\n" +
			        			"Êîíòàêòû: " + visit.getUserContacts() + "\n" +
			        			"Óñëóãà: " + p + "\n" +
			        			"Ìàñòåð: " + s + "\n" + "\n" + 
